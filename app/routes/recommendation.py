...
@recommendation_bp.route("/cekhistory", methods=["GET"])
...
@recommendation_bp.route("/ceknewnews", methods=["GET"])
def get_preprocessing_new_news(email):
    ...
    # path ke database berita baru
    path = "D:/KULIAH/polban_smt_6/ta/test_rekom_form/be-newsmeter-mobile/app/utils/preprocessed_data.csv"
        ...
        if not df:
            ...
            for i in range(0, len(df), len(df)):
                ...
                for j in range(len(history_list)):
                    ...
                    # mengambil 3 berita dengan nilai tertinggi
                    # result = {}
                    ...
                    # mengambil 1 nilai tertinggi
                    result = {}
                    result["recommendation"] = []
                    recs = results.get(1, [])  # Ambil semua hasil similarity score
                    recs_top = recs[:1]  # Ambil 3 nilai teratas dari similarity score

                    for rec in recs_top:
                        recommendation = {}
                        recommendation["score"] = round(rec[0], 1)
                        index = rec[1] - 1
                        # Get the values of the columns that you need
                        recommendation["id_history"] = str(concat_df.loc[0, "_id"])
                        recommendation["id_has_read"] = concat_df.loc[0, "id_has_read"]
...
def save_recommendation():
    email = "salsabilaputriiiii29@gmail.com"
